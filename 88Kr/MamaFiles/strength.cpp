{
	...
	TH2F *h = new TH2F("h"," ",10,0.0, 14,10,1.0e-9,5.0e-07);
   ...
   float a1 =   0.1280;
	...
	for (int i = 0; i < strengthexp->GetN(); i++)
		{
			std::cout << strengthexp->GetX()[i]*1000 <<" " << strengthexp->GetY()[i] - strengthexp->GetEY()[i]  << " " << strengthexp->GetY()[i] + strengthexp->GetEY()[i]<<std::endl;
		}
	...
	TLatex t;
	t.SetTextSize(0.05);
	t.DrawLatex(    1.318,1.646e-07,"^{xx}Yy");
	c1->Update();
