{
   ...
   TH2F *h = new TH2F("h"," ",10,-0.708000,6.460000,50,0.019514,8000.000000);
   ifstream rholev("rholev.cnt"), rhopaw("rhopaw.cnt"), fermi("fermigas.cnt");
   float levels[59],rho[59],rhoerr[59],energy[783],energyerr[783],fermigas[783];
   float Bn[1]={6.000000};
   float Bnerr[1]={0.001};
   float rho_Bn[1]={800.000000};
   float rho_Bnerr[1]={80.000000};
   int i = 0;
   float a0 =  -0.7080;
   float a1 =   0.1280;
   float x,y,z;
   ...
   for (int i = 0; i < rhoexp->GetN(); i++)
   {
   	std::cout << rhoexp->GetX()[i] <<" " << rhoexp->GetY()[i]  << " " << rhoexp->GetEY()[i] <<std::endl;
   }
   c1->SetLogy();
   ...
   fermicalc->DrawGraph(21,&fermicalc->GetX()[36],&fermicalc->GetY()[36],"L");
   ...
   TLatex t;
   t.SetTextSize(0.05);
   t.DrawLatex(    5.168,4.000e+03,"^{xx}Yy");
   TArrow *arrow1 = new TArrow(0.060000,23.886459,0.060000,9.723951,0.02,">");
   arrow1->Draw();
   TArrow *arrow2 = new TArrow(3.004000,170.388715,3.004000,69.363632,0.02,">");
   arrow2->Draw();
   TArrow *arrow3 = new TArrow(4.412000,648.744663,4.412000,264.097809,0.02,">");
   arrow3->Draw();
   TArrow *arrow4 = new TArrow(4.924000,1793.362254,4.924000,730.060791,0.02,">");
   arrow4->Draw();
   c1->Update();
